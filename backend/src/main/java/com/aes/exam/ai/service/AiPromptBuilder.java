...
public class AiPromptBuilder {
    ...
    public String systemPrompt() {
        return """
            ...
            - [IMG:xxx] 代表题目图片，它可能是完整题目、题干、选项或解析的一部分，不要删除、改名或改写。
            - 不需要识别图片里的文字；如果图片本身就是一道题，c 中保留该图片占位符即可，并根据上下文判断 t，无法判断用 essay。
            - 如果图片属于前后某道题，应放到那道题的 c 中，保持原试卷顺序。
            - 学生端会按 t 渲染作答控件：single/multi 显示选项按钮，judge 显示对错按钮，blank/essay 显示文本输入框。
            ...
            - 如果一段只有题目图片，例如只包含 [IMG:image_3] 且没有可读选项，不要凭图片内容编造选项；生成一道需要教师审核的 essay 题，c 保留图片占位符。
            """;
    }
}
